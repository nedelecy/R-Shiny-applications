# Entry point run by GitHub Actions (or manually: Rscript scripts/update_data.R)
source("R/api.R")

dir.create("data", showWarnings = FALSE)

# ---- 1. Fetch ---------------------------------------------------------------
df <- fetch_data(endpoint = "items")

# ---- 2. Validate (fail the job rather than overwrite good data with bad) ----
stopifnot(
  "API returned no rows" = nrow(df) > 0
...
file.copy(tmp, "data/items.rds", overwrite = TRUE)

# ---- 4. Save a small metadata file the Shiny app can display ----------------
saveRDS(
  list(updated_at = Sys.time(), n_rows = nrow(df)),
  "data/meta.rds"
)
...
#  }
  ...
  tmp <- tempfile(fileext = ".rds")
  saveRDS(df, tmp, compress = "xz")
  file.copy(tmp, paste("data/",dataset,".rds", sep = ""), overwrite = TRUE)
  
  # ---- 4. Save a small metadata file the Shiny app can display ----------------
  saveRDS(list(updated_at = Sys.time(), n_rows = nrow(df)), "data/meta.rds")
